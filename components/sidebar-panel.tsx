  ...
  const metrics = [
    {
      label: "Level",
      value:
        profile.currentDifficulty.charAt(0).toUpperCase() +
        profile.currentDifficulty.slice(1),
      icon: Target,
      color: "text-blue-500",
    },
    {
    ...
    },
    {
      label: "Avg Time",
      value:
        profile.averageTimePerQuestion > 0
          ? `${Math.round(profile.averageTimePerQuestion)}s`
          : "—",
      icon: Clock,
    ...
    >
          ...
          <div className="bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-lg p-4 border border-blue-200 dark:border-blue-800">
            ...
            <p
              className={`text-2xl font-bold ${
                isPaused ? "text-orange-600" : "text-blue-600"
              }`}
            >
              {currentTimerDisplay}
            </p>
            {isPaused && (
              <p className="text-xs text-orange-600 mt-1">⏸ Paused</p>
            )}
          </div>
      ...
      {isCollapsed && (
        <div className="space-y-4 flex flex-col items-center">
          <div
            className="w-8 h-8 rounded-md bg-blue-500/20 flex items-center justify-center"
            title="Level"
          >
            <Target className="w-4 h-4 text-blue-500" />
          </div>
          <div
            className="w-8 h-8 rounded-md bg-green-500/20 flex items-center justify-center"
            title="Success"
          >
            <TrendingUp className="w-4 h-4 text-green-500" />
          </div>
          <div
            className="w-8 h-8 rounded-md bg-purple-500/20 flex items-center justify-center"
            title="Attempts"
          >
            <Zap className="w-4 h-4 text-purple-500" />
          </div>
          <div
            className="w-8 h-8 rounded-md bg-orange-500/20 flex items-center justify-center"
            title="Avg Time"
          >
            <Clock className="w-4 h-4 text-orange-500" />
          </div>
          <div
            className="w-8 h-8 rounded-md bg-indigo-500/20 flex items-center justify-center"
            title="Total Time"
          >
