...
export function calculateLearningAnalytics(
  ...
  const topicPerformance = Array.from(profile.topicStats.entries()).map(
    ([topicId, stats]) => ({
      topicId,
...
export function updateUserProfile(
  ...
  const { addAnsweredQuestion } = require("./question-store");
  const {
    trackWrongSubmission,
    resetWrongSubmissionStreak,
  } = require("./wrong-submission-handler");
