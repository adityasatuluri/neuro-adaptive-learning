...
export function calculatePerformanceMomentum(
  ...
  const topicAttempts = topicId
    ? recentAttempts.filter((p) => {
        const question = profile.progressHistory.find((h) => h.questionId === p.questionId)
        return question?.difficulty !== undefined
      })
    : recentAttempts

  if (topicAttempts.length < 5) {
    return { momentum: 0, trend: "stable", recentAccuracy: 0, previousAccuracy: 0 }
  }
  ...
  const firstHalfAccuracy = (firstHalf.filter((a) => a.correct).length / firstHalf.length) * 100
  const secondHalfAccuracy = (secondHalf.filter((a) => a.correct).length / secondHalf.length) * 100
...
export function calculateTimeBasedDifficultyAdjustment(profile: UserProfile, topicId?: string): number {
  const recentAttempts = profile.progressHistory.slice(-20)
  if (recentAttempts.length < 5) return 1

  const avgTimePerQuestion = recentAttempts.reduce((sum, a) => sum + a.timeSpent, 0) / recentAttempts.length
  const estimatedTime = 600 // 10 minutes is ideal

  // If taking too long, reduce difficulty to build confidence
  if (avgTimePerQuestion > estimatedTime * 2) return 0.8
  if (avgTimePerQuestion > estimatedTime * 1.5) return 0.9

  // If solving quickly and correctly, increase difficulty
  const recentCorrect = recentAttempts.filter((a) => a.correct).length
  if (recentCorrect / recentAttempts.length > 0.8 && avgTimePerQuestion < estimatedTime * 0.7) {
    return 1.2
  }
...
export function calculateConfidenceBasedAdjustment(profile: UserProfile): number {
  const recentAttempts = profile.progressHistory.slice(-10)
  if (recentAttempts.length === 0) return 1

  const avgConfidence = recentAttempts.reduce((sum, a) => sum + a.confidence, 0) / recentAttempts.length

  // Low confidence: reduce difficulty
  if (avgConfidence < 30) return 0.75
  if (avgConfidence < 50) return 0.9

  // High confidence: increase difficulty
...
export function calculatePerformanceMetrics(profile: UserProfile, topicId?: string): PerformanceMetrics {
  ...
  for (let i = 0; i < topicAttempts.length; i += 10) {
    ...
    const windowAccuracy = (window.filter((a) => a.correct).length / window.length) * 100
    recentWindows.push(windowAccuracy)
  }
  const avgAccuracy = recentWindows.reduce((a, b) => a + b, 0) / recentWindows.length
  const variance = recentWindows.reduce((sum, acc) => sum + Math.pow(acc - avgAccuracy, 2), 0) / recentWindows.length
  const consistency = Math.max(0, 100 - Math.sqrt(variance))

  const firstHalf = topicAttempts.slice(0, Math.floor(topicAttempts.length / 2))
  const secondHalf = topicAttempts.slice(Math.floor(topicAttempts.length / 2))
  const firstHalfAccuracy = (firstHalf.filter((a) => a.correct).length / firstHalf.length) * 100
  const secondHalfAccuracy = (secondHalf.filter((a) => a.correct).length / secondHalf.length) * 100
  const learningVelocity = Math.max(-100, Math.min(100, secondHalfAccuracy - firstHalfAccuracy))

  const errorPatterns = new Map<string, number>()
  topicAttempts.forEach((attempt) => {
    if (!attempt.correct && attempt.errorType) {
      errorPatterns.set(attempt.errorType, (errorPatterns.get(attempt.errorType) || 0) + 1)
    }
  })
  ...
  topicAttempts.forEach((attempt) => {
    attempt.conceptsInvolved?.forEach((concept) => {
      ...
      conceptMastery.set(concept, ((current.correct / current.total) * 100) as any)
    })
  })
...
export function calculateLearningAnalytics(profile: UserProfile): LearningAnalytics {
  const metrics = calculatePerformanceMetrics(profile)
  const allAttempts = profile.progressHistory

  let learningStyle: "fast-learner" | "steady-learner" | "struggling" = "steady-learner"
  ...
  const topicPerformance = Array.from(profile.topicStats.entries()).map(([topicId, stats]) => ({
    topicId,
    accuracy: stats.averageAccuracy,
  }))
  topicPerformance.sort((a, b) => a.accuracy - b.accuracy)

  const weakAreas = topicPerformance.slice(0, 3).map((t) => t.topicId)
  const strongAreas = topicPerformance.slice(-3).map((t) => t.topicId)
  ...
  const avgSessionDuration = allAttempts.length > 0 ? profile.totalTimeSpent / sessions.size : 0
...
export function calculateNextDifficulty(profile: UserProfile): "easy" | "medium" | "hard" {
  ...
  const baseScore = accuracyScore * 0.4 + speedScore * 0.2 + consistencyScore * 0.25 + velocityScore * 0.15
  ...
  const adjustedScore = baseScore * streakMultiplier * timeMultiplier * confidenceMultiplier
...
export function calculateNextReviewDate(reviewCount: number, metrics?: PerformanceMetrics): number {
  const baseInterval = SPACED_REPETITION_INTERVALS[Math.min(reviewCount, SPACED_REPETITION_INTERVALS.length - 1)]
...
export function selectNextQuestion(questions: Question[], profile: UserProfile, currentTopic: string): Question | null {
  ...
  const allQuestions = [...questions, ...csvQuestions, ...getCachedAIQuestions(profile.currentDifficulty)]
  ...
  if (dueForReview.length > 0) {
    const sorted = dueForReview.sort((a, b) => {
      const aConceptMastery =
        Array.from(metrics.conceptMastery.values()).reduce((a, b) => a + b, 0) / metrics.conceptMastery.size || 0
  ...
  return (
    allQuestions.find((q) => q.topic === currentTopic && !profile.progressHistory.some((p) => p.questionId === q.id)) ||
    null
  )
}
...
export function selectRandomQuestionSameDifficulty(
  ...
  const availableQuestions = allQuestions.filter(
    (q) => q.topic === currentTopic && q.difficulty === profile.currentDifficulty && q.id !== excludeQuestionId,
  )
...
export function updateUserProfile(
  ...
  if (isCorrect) {
    ...
    conceptsInvolved.forEach((concept) => {
      const current = topicStats.conceptBreakdown.get(concept) || { attempted: 0, correct: 0 }
      current.attempted += 1
      current.correct += 1
  ...
  if (today !== lastActivityDay) {
    if (new Date(lastActivityDate).getTime() + 24 * 60 * 60 * 1000 > Date.now()) {
