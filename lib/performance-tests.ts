  ...
  return runTestSuite("Performance Tests", [
    [
      ...
      () => {
        ...
        for (let i = 0; i < 100; i++) {
          ...
          updateUserProfile(profile, question, Math.random() > 0.3, 120, "code", 70)
        }

        const duration = performance.now() - startTime
        console.log(`[v0] Processed 100 questions in ${duration.toFixed(2)}ms`)
        assert.isTrue(duration < 5000, "Should process 100 questions in under 5 seconds")
      },
    ],
    ...
    [
      ...
      () => {
        ...
        const duration = performance.now() - startTime
        console.log(`[v0] Calculated metrics in ${duration.toFixed(2)}ms`)
        assert.isTrue(duration < 1000, "Should calculate metrics in under 1 second")
      },
    ],
    ...
    [
      ...
      () => {
        ...
        console.log(`[v0] Generated 10 questions in ${duration.toFixed(2)}ms`)
      },
    ],
  ])
}
