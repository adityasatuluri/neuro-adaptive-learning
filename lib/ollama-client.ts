...
// Fixed: Proper model loading, endpoint validation, and error handling
...
async function checkAndLoadModel(): Promise<boolean> {
  try {
    ...
    const modelExists = models.some((m: any) => m.name.includes(OLLAMA_MODEL));

    if (!modelExists) {
      console.warn(
        `[v0] Model '${OLLAMA_MODEL}' not found. Available models:`,
        models.map((m: any) => m.name)
      );
      if (models.length > 0) {
    ...
    console.log(`[v0] Model '${OLLAMA_MODEL}' is available`);
    return true;
  } catch (error) {
...
async function callOllama(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(
        `[v0] Ollama attempt ${attempt}/${maxRetries} with model: ${OLLAMA_MODEL}`
      );
