...
export async function generatePersonalizedLearningPath(profile: UserProfile): Promise<LearningPath | null> {
  // Determine user level based on current difficulty and accuracy
  let userLevel: "beginner" | "intermediate" | "advanced" = "beginner"
  if (profile.currentDifficulty === "medium" && profile.performanceMetrics.accuracy > 70) {
    userLevel = "intermediate"
  } else if (profile.currentDifficulty === "hard" && profile.performanceMetrics.accuracy > 75) {
    userLevel = "advanced"
  }
  ...
  console.log("[v0] Generating personalized learning path for", userLevel, "learner")
  console.log("[v0] Weak areas:", weakAreas)
  console.log("[v0] Strong areas:", strongAreas)

  const generatedPath = await generateLearningPath(userLevel, weakAreas, strongAreas)

  if (!generatedPath) {
    console.error("[v0] Failed to generate learning path")
    return null
  }
...
export function updateLearningPathBasedOnProgress(profile: UserProfile, currentPath: LearningPath): LearningPath {
...
export function shouldRegenerateLearningPath(profile: UserProfile, lastPathGenerationTime: number): boolean {
  // Regenerate path if:
  // 1. More than 7 days have passed
  // 2. User has completed a topic
  // 3. User's accuracy has changed significantly (>15%)

  const daysSinceGeneration = (Date.now() - lastPathGenerationTime) / (1000 * 60 * 60 * 24)

  if (daysSinceGeneration > 7) {
    console.log("[v0] Regenerating path: 7+ days have passed")
    return true
  }

  if (profile.topicsCompleted.length > 0) {
    console.log("[v0] Regenerating path: topics completed")
    return true
  }
