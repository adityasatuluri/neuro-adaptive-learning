...
async function callOllama(prompt: string, model: string = OLLAMA_MODEL): Promise<string | null> {
  try {
...
- Only fail if logic is fundamentally wrong`
    ...
    try {
      ...
      console.log("[v0] FINAL AI RESPONSE (Code Validation):", JSON.stringify(parsed, null, 2))
      const validated = codeValidationSchema.parse(parsed)
      console.log("[v0] Code validation completed:", validated.passed ? "PASSED" : "FAILED")
      return validated
    } catch (error) {
