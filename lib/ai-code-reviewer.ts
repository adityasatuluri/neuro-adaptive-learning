...
async function callOllama(
  ...
  try {
    console.log(" Calling Ollama for code validation with model:", model);
    ...
    console.log(" Ollama validation response received");
    return data.response || null;
  } catch (error) {
    console.error(" Error calling Ollama for validation:", error);
    return null;
  }
}
...
function extractJsonFromResponse(content: string): string | null {
  ...
  if (jsonStart === -1 || jsonEnd === -1) {
    console.warn(" No JSON found in validation response");
    return null;
  }
...
- Only fail if logic is fundamentally wrong`;
    ...
    if (!response) {
      console.error(" No response from Ollama for validation");
      return null;
    }

    const jsonStr = extractJsonFromResponse(response);
    if (!jsonStr) {
      console.error(" Could not extract JSON from validation response");
      return null;
    }
    ...
    try {
      ...
      console.log(
        " FINAL AI RESPONSE (Code Validation):",
